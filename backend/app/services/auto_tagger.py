...
def _hf_cache_dir() -> Path:
    ...
    Defaults to the app's models dir. But older NekoBooru versions downloaded to
    the default ``~/.cache/huggingface`` cache, so after an upgrade the app would
    otherwise look in a fresh (empty) location and report already-downloaded
    models as missing. To avoid orphaning them, fall back to the legacy default
    cache when the app dir has no NekoBooru models and the legacy one does.
    """
